...
import { MessageCircle, X, Send, Loader2, ExternalLink, RefreshCw } from 'lucide-react'
...
export function ChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    ...
    try {
      const saved = localStorage.getItem('bk_chat_history')
      if (saved) {
  ...
  useEffect(() => {
    try {
      // Keep last 10 messages to avoid exceeding localStorage limit
  ...
  // Handle re-generating the last assistant response (P2)
